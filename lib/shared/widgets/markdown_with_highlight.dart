...
class MarkdownWithCodeHighlight extends StatefulWidget {
  ...
  String preprocessFencesForTesting(
    String input, {
  ...
  String repairMalformedFencesForTesting(String input) {
...
const List<String> _malformedFenceLanguagePrefixes = <String>[
  'objectivec',
  'typescript',
  'javascript',
  'dockerfile',
...
final String _malformedFenceLanguagePattern = _malformedFenceLanguagePrefixes
    .map(RegExp.escape)
    .join('|');

final RegExp _malformedFenceOpenWithSpaceRe = RegExp(
  r'^([ \t]{0,3})([`~]{3,})(' +
      _malformedFenceLanguagePattern +
      r')([ \t]+)(\S.*)$',
  multiLine: true,
...
final RegExp _malformedFenceOpenAttachedRe = RegExp(
  r'^([ \t]{0,3})([`~]{3,})(' + _malformedFenceLanguagePattern + r')(\S.*)$',
  multiLine: true,
  caseSensitive: false,
);

final RegExp _malformedFenceWithPrefixSpaceRe = RegExp(
  r'^([ \t]*\S.*?)([`~]{3,})(' +
      _malformedFenceLanguagePattern +
      r')([ \t]+)(\S.*)$',
  multiLine: true,
...
final RegExp _malformedFenceWithPrefixAttachedRe = RegExp(
  r'^([ \t]*\S.*?)([`~]{3,})(' + _malformedFenceLanguagePattern + r')(\S.*)$',
  multiLine: true,
  caseSensitive: false,
);
...
({String prefix, String fenceToken, String language, String body})?
_matchMalformedFenceLine(String line) {
  ...
  final prefixedAttached = _malformedFenceWithPrefixAttachedRe.firstMatch(line);
...
String _preprocessFences(
  ...
  // STEP 0: Normalize malformed fenced code blocks before masking.
  // Keeps repair isolated from the later code masking pipeline and avoids
  // placeholder leakage such as __CODE_MASK_* ending up in output.
