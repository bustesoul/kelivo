...
String _preprocessFences(
  ...
  // Tolerate missing space after ATX hashes, common in LLM CJK output.
...
String _normalizeMalformedFences(String input) {
  final lines = input.split('\n');
  final normalized = <String>[];
  var inValidFence = false;
  var validFenceChar = '';
  var validFenceLength = 0;
  var inRepairedFence = false;
  var repairedFenceToken = '';
  ...
  for (final line in lines) {
    ...
    if (malformed != null) {
      ...
      if (prefixText.isNotEmpty) normalized.add(prefixText);
...
({String body, String? closingFence}) _splitTrailingClosingFence(
  String text,
  String fenceToken,
) {
  final body = text.trimRight();
  if (body.isEmpty) return (body: body, closingFence: null);
  ...
  if (match == null) return (body: body, closingFence: null);
  final codeBody = (match.group(1) ?? '').trimRight();
  final closingFence = (match.group(2) ?? '').trim();
  if (closingFence.isEmpty) return (body: body, closingFence: null);
  return (body: codeBody, closingFence: closingFence);
}

_MalformedFenceLine? _matchMalformedFenceLine(String line) {
  const language = r'([A-Za-z][A-Za-z0-9_+#.-]*)';
  final patterns = <RegExp>[
    RegExp(r'^([ \t]{0,3})([`~]{3,})' + language + r'[ \t]+(\S.*)$'),
    RegExp(r'^([ \t]*\S.*?)([`~]{3,})' + language + r'[ \t]+(\S.*)$'),
  ];
  for (final pattern in patterns) {
    final match = pattern.firstMatch(line);
    if (match == null) continue;
    final normalized = _splitMalformedFenceLanguage(match[3]!, match[4]!);
    return _MalformedFenceLine(
      prefix: match[1]!,
      fenceToken: match[2]!,
...
({String language, String body}) _splitMalformedFenceLanguage(
  String rawLanguage,
  String rawBody,
) {
  ...
  final language = rawLanguage.toLowerCase();
  final candidates =
      knownLanguages
          .where((candidate) => language.startsWith(candidate))
          .toList()
        ..sort((a, b) => b.length.compareTo(a.length));
  if (candidates.isEmpty) {
    return (language: rawLanguage, body: rawBody);
  }

  final prefix = candidates.first;
  if (prefix.length == rawLanguage.length) {
    return (language: rawLanguage, body: rawBody);
  }

  final attached = rawLanguage.substring(prefix.length);
  final body = rawBody.isEmpty ? attached : '$attached $rawBody';
  return (language: rawLanguage.substring(0, prefix.length), body: body);
}

({String fenceChar, int fenceLength})? _parseFenceOpenerLine(String line) {
  ...
  var fenceLength = 0;
  while (fenceLength < stripped.length && stripped[fenceLength] == fenceChar) {
    fenceLength++;
  }
  if (fenceLength < 3) return null;
  final rest = stripped.substring(fenceLength).trim();
  if (fenceChar == '`' && rest.contains('`')) return null;
  return (fenceChar: fenceChar, fenceLength: fenceLength);
}

bool _isFenceClosingLine(String line, String fenceChar, int minLength) {
...
class _MalformedFenceLine {
  const _MalformedFenceLine({
